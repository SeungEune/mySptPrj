...
public class UserController {
     ...
     * @return 사용자 수정 페이지
     */
    @PostMapping("/userEditForm.do")
    public String userEditForm(@RequestParam String userId, Model model) {
        // userId 필수 검증
    ...
    public ResponseEntity saveUser(@RequestBody UserVO userVO) {
        ...
        try {
            // 필수 입력값 검증
            ResponseEntity validationError = validateUserForSave(userVO);
    ...
    public ResponseEntity updateUser(@RequestBody UserVO userVO) {
        ...
        try {
            // 필수 입력값 검증
            ResponseEntity validationError = validateUserForUpdate(userVO);
            if (validationError != null) {
                return validationError;
            }
            
            // 사용자 정보 수정
            ResultVO resultVO = userService.updateUser(userVO);
            ...
            return ApiResponseVO.apiResponse(null, HttpStatus.INTERNAL_SERVER_ERROR.value(), "사용자 수정 중 오류가 발생했습니다.");
        }
    }

    /**
     * 사용자 삭제 (논리 삭제: use_yn='N')
     * @param params 사용자 ID
     * @return 삭제 결과
     */
    @ResponseBody
    ...
    public ResponseEntity deleteUser(@RequestBody Map<String, String> params) {
        ...
        try {
            ...
            return ApiResponseVO.apiResponse(null, HttpStatus.INTERNAL_SERVER_ERROR.value(), "사용자 삭제 중 오류가 발생했습니다.");
        }
    }
     ...
     * 사용자 등록 시 필수 입력값 검증
     * @param userVO 사용자 정보
     * @return 검증 실패 시 에러 응답, 성공 시 null
     */
    private ResponseEntity validateUserForSave(UserVO userVO) {
     ...
     * @return 검증 실패 시 에러 응답, 성공 시 null
     */
    private ResponseEntity validateUserForUpdate(UserVO userVO) {
        if (StringUtil.isEmpty(userVO.getUserId())) {
            return ApiResponseVO.apiResponse(null, HttpStatus.BAD_REQUEST.value(), "사용자ID는 필수입니다.");
        }
        if (StringUtil.isEmpty(userVO.getUserNm())) {
