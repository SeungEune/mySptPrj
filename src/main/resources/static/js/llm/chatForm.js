...
function fn_bindEvent() {
    ...
    document.getElementById('runPerfTestBtn').addEventListener('click', fn_runPerfTest);
...
function fn_createNewChatSession() {
    ...
    const title = promptRoleList.find(item => item.roleCd === roleCd)?.roleNm + ' 대화';
    ...
    callModule.call(Util.getRequestUrl('/llm/createSession.do'), {
        ...
        fn_loadSessionList();
        MessageUtil.toast('새 대화가 시작되었습니다.');
    }, true, 'POST');
}
...
function fn_runPerfTest() {
    const prompt = Util.isEmpty(document.getElementById('perfPrompt').value) ? '' : document.getElementById('perfPrompt').value.trim();
    const concurrency = Number(document.getElementById('perfConcurrency').value);
    const requestCount = Number(document.getElementById('perfRequestCount').value);

    if (Util.isEmpty(prompt)) {
        MessageUtil.alert('테스트 프롬프트를 입력해주세요.');
        return;
    }
    ...
    document.getElementById('runPerfTestBtn').disabled = true;
    document.getElementById('perfSummaryArea').textContent = '성능 테스트를 실행 중입니다. 잠시만 기다려주세요.';

    callModule.call(Util.getRequestUrl('/llm/runPerfTest.do'), requestData, (result) => {
        document.getElementById('runPerfTestBtn').disabled = false;

        if (result && result.status && result.status.code !== 200) {
            document.getElementById('perfSummaryArea').textContent = Util.nvl(result.status.message, '성능 테스트 중 오류가 발생했습니다.');
            return;
        }

        const response = result ? result.result : null;
        if (!response || !response.summary) {
            document.getElementById('perfSummaryArea').textContent = '성능 테스트 결과를 확인할 수 없습니다.';
            return;
        }

        fn_renderPerfSummary(response.summary);
        fn_renderPerfResultList(response.resultList || []);
    }, true, 'POST');
}

function fn_renderPerfSummary(summary) {
    const summaryArea = document.getElementById('perfSummaryArea');
    summaryArea.innerHTML = ''
        + '<strong>모델:</strong> ' + summary.model
        + ' | <strong>동시 요청 수:</strong> ' + summary.concurrency
        + ' | <strong>총 요청 수:</strong> ' + summary.requestCount
        + ' | <strong>성공:</strong> ' + summary.successCount
        + ' | <strong>실패:</strong> ' + summary.failCount
        + ' | <strong>평균 응답시간:</strong> ' + summary.avgElapsedMs + 'ms'
        + ' | <strong>최소:</strong> ' + summary.minElapsedMs + 'ms'
        + ' | <strong>최대:</strong> ' + summary.maxElapsedMs + 'ms'
        + ' | <strong>성공률:</strong> ' + Number(summary.successRate).toFixed(2) + '%';
}

function fn_renderPerfResultList(resultList) {
    $('#perfResultBody').empty();
    if (resultList.length > 0) {
        $('#perfResultTemplate').tmpl(resultList).appendTo('#perfResultBody');
    } else {
        $('#perfEmptyTemplate').tmpl().appendTo('#perfResultBody');
    }
}
